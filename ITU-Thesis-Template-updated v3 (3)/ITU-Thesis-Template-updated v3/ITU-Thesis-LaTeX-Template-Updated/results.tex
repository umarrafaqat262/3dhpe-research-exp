...
motivated in Section~\ref{sec:intro-objectives}, both of which are left as
future work and are discussed as such. The main Human3.6M evaluation is
reported in full. Section~\ref{sec:results-summary} closes the chapter.
...
PoseMamba-S in the same tier. Against that baseline KinecMamba lowers the
MPJPE from 41.8mm to 41.55mm and the P-MPJPE from 35.0mm to 34.52mm, an
improvement on both metrics at a comparable parameter budget. It is also
competitive with the other compact state-space model, SasMamba (41.48mm P1 /
34.84mm P2): marginally higher on P1 and lower on P2. The larger baselines
in the table (MotionBERT, MotionAGFormer-L, Pose Magic, PoseMamba-L) reach
lower MPJPE, but at five to thirty times the parameter count, placing them in
a different capacity tier. Section~\ref{sec:results-ablation} discusses the
...
\caption[Accuracy versus model size on Human3.6M.]{Accuracy (MPJPE, lower is
  ...
  state-space tier and improves on its closest baseline PoseMamba-S (0.9M,
  41.8mm) at a comparable parameter budget.}
...
The cross-dataset evaluation on MPI-INF-3DHP is left as future work. The
data pipeline for this benchmark is prepared, but the evaluation was not
completed within the scope of this thesis, whose main empirical claim rests
on the Human3.6M comparison in Section~\ref{sec:results-h36m}. Reporting
MPJPE, PCK, and AUC on MPI-INF-3DHP is listed among the recommendations in
...
41.55mm MPJPE and 34.52mm P-MPJPE, improving on its closest baseline
PoseMamba-S (41.8mm / 35.0mm) at a comparable parameter budget while
remaining competitive with the compact state-space model SasMamba. The
cross-dataset evaluation on MPI-INF-3DHP
(Section~\ref{sec:results-mpiinf3dhp}) and the scan-order ablation that
would isolate the contribution of the BFS kinematic-tree scan
(Section~\ref{sec:results-ablation}) are left as future work.
