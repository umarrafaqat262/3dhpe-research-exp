...
derives its spatial scan order directly and deterministically from the
skeleton's own kinematic tree (Section~\ref{sec:lit-summary-gap}). This
chapter presents an architecture that closes this gap.

The proposed architecture is a dual-stack, Mamba-based network that maps a
...
otherwise used by this cross-scan. Section~\ref{sec:method-loss} defines
...
neck, 9 nose, 10 head, 11 left shoulder, 12 left elbow, 13 left wrist, 14
right shoulder, 15 right elbow, and 16 right wrist.
...
\centering
\begin{tikzpicture}[
  joint/.style={draw, rounded corners, fill=white, minimum width=2.1cm,
    minimum height=0.7cm, font=\footnotesize, align=center},
  root/.style={joint, fill=black!12, thick},
  bone/.style={-, thick}
]
 \node[root] (n0)  at (0,0)      {0: Pelvis};

 \node[joint] (n1)  at (-3.2,-1.4)  {1: R.\ Hip};
 \node[joint] (n7)  at (0,-1.4)     {7: Spine};
 \node[joint] (n4)  at (3.2,-1.4)   {4: L.\ Hip};

 \node[joint] (n2)  at (-3.2,-2.8)  {2: R.\ Knee};
 \node[joint] (n8)  at (0,-2.8)     {8: Neck};
 \node[joint] (n5)  at (3.2,-2.8)   {5: L.\ Knee};

 \node[joint] (n3)  at (-3.2,-4.2)  {3: R.\ Ankle};
 \node[joint] (n14) at (-1.6,-4.2)  {14: R.\ Shoulder};
 \node[joint] (n9)  at (0,-4.2)     {9: Nose};
 \node[joint] (n11) at (1.6,-4.2)   {11: L.\ Shoulder};
 \node[joint] (n6)  at (3.2,-4.2)   {6: L.\ Ankle};

 \node[joint] (n15) at (-2.6,-5.6)  {15: R.\ Elbow};
 \node[joint] (n10) at (0,-5.6)     {10: Head};
 \node[joint] (n12) at (2.6,-5.6)   {12: L.\ Elbow};

 \node[joint] (n16) at (-2.6,-7.0)  {16: R.\ Wrist};
 \node[joint] (n13) at (2.6,-7.0)   {13: L.\ Wrist};

 \draw[bone] (n0) -- (n1);  \draw[bone] (n1) -- (n2);  \draw[bone] (n2) -- (n3);
 \draw[bone] (n0) -- (n4);  \draw[bone] (n4) -- (n5);  \draw[bone] (n5) -- (n6);
 \draw[bone] (n0) -- (n7);  \draw[bone] (n7) -- (n8);
 \draw[bone] (n8) -- (n9);  \draw[bone] (n9) -- (n10);
 \draw[bone] (n8) -- (n11); \draw[bone] (n11) -- (n12); \draw[bone] (n12) -- (n13);
 \draw[bone] (n8) -- (n14); \draw[bone] (n14) -- (n15); \draw[bone] (n15) -- (n16);
\end{tikzpicture}
...
The proposed network is a dual-stack, Mamba-based architecture that
...
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners, fill=white, minimum width=7.2cm,
    minimum height=0.9cm, font=\footnotesize, align=center},
  stage/.style={draw, rounded corners, fill=black!5, minimum width=6cm,
    minimum height=0.8cm, font=\scriptsize, align=center},
  arr/.style={-{Latex[length=2.2mm]}, thick}
]
 \node[block] (input) at (0,0)     {Input $X \in \mathbb{R}^{B,T,J,2}$};
 \node[block] (ste)   at (0,-1.3)  {Spatial Token Embedding (STE)\\ $\mathrm{Linear}(2{\to}C)$ + spatial positional embedding};
 \node[block] (tte)   at (0,-2.9)  {Temporal Token Embedding (TTE)\\ $+$ temporal positional embedding};

 \node[stage] (sp) at (0,-4.6) {Spatial BiSTSSMBlock};
 \node[stage] (tp) at (0,-5.5) {Temporal BiSTSSMBlock};

 \begin{scope}[on background layer]
   \node[draw, dashed, rounded corners, fit=(sp)(tp), inner sep=6pt,
     label={[font=\footnotesize]above right:$\times\, 10$}] (loopbox) {};
 \end{scope}

 \node[block] (head)   at (0,-7.3) {Output head\\ $\mathrm{LayerNorm} \to \mathrm{Linear}(C{\to}3)$};
 \node[block] (output) at (0,-8.6) {Output $\hat{Y} \in \mathbb{R}^{B,T,J,3}$};

 \draw[arr] (input) -- (ste);
 \draw[arr] (ste) -- (tte);
 \draw[arr] (tte) -- (loopbox.north);
 \draw[arr] (sp) -- (tp);
 \draw[arr] (loopbox.south) -- (head);
...
\centering
\begin{tikzpicture}[
  block/.style={draw, rounded corners, fill=white, minimum width=7.4cm,
    minimum height=0.8cm, font=\footnotesize, align=center},
  stage/.style={draw, rounded corners, fill=black!5, minimum width=6.2cm,
    minimum height=0.75cm, font=\scriptsize, align=center},
  arr/.style={-{Latex[length=2.2mm]}, thick}
]
 \node[block] (in)   at (0,0)     {Input $Z \in \mathbb{R}^{T,J,C}$ (post-LayerNorm)};
 \node[block] (scan) at (0,-1.3)  {Four-directional cross-scan\\ unfold $T\times J$ grid into 4 sequences};

 \node[stage] (proj1) at (0,-2.8) {Input projection: $\mathrm{Linear}(C{\to}2C)$};
 \node[stage] (conv)  at (0,-3.7) {Depthwise Conv1D (kernel $=3$)};
 \node[stage] (ssm)   at (0,-5.0) {Selective SSM: project $\Delta, B, C$;\\ scan with learned $A \in \mathbb{R}^{N\times N}$};

 \begin{scope}[on background layer]
   \node[draw, dashed, rounded corners, fit=(proj1)(conv)(ssm), inner sep=6pt,
     label={[font=\footnotesize]above right:$\times\,4$ (one per direction)}] (loopbox) {};
 \end{scope}

 \node[block] (merge) at (0,-6.8) {Cross-merge: recombine 4 directional outputs};
 \node[block] (proj2) at (0,-8.1) {Output projection: $\mathrm{Linear}(2C{\to}C)$};
 \node[block] (out)   at (0,-9.3) {Output $O \in \mathbb{R}^{T,J,C}$};

 \draw[arr] (in) -- (scan);
 \draw[arr] (scan) -- (loopbox.north);
 \draw[arr] (proj1) -- (conv);
 \draw[arr] (conv) -- (ssm);
 \draw[arr] (loopbox.south) -- (merge);
 \draw[arr] (merge) -- (proj2);
 \draw[arr] (proj2) -- (out);
\end{tikzpicture}
...
layer unfolds this grid into four separate 1D sequences by traversing it
along its two natural flattenings, each in both directions. The first
flattening is row-major (frame-major, sweeping all joints within a frame
before advancing to the next frame) and the second is column-major
(joint-major, sweeping all frames for a joint before advancing to the next
joint), and the four resulting sweeps run top-left to bottom-right,
bottom-right to top-left, top-right to bottom-left, and bottom-left to
top-right. Each of these four 1D sequences is processed by
an independent instance of the remaining steps below, so that every token
is eventually informed by grid neighbors reached from all four traversal
directions, not only those preceding it in a single fixed order.
...
sequences (where $L=TJ$) is then processed by a selective state space
model, following the continuous-time linear formulation reviewed in
Section~\ref{sec:lit-ssm-foundations} \cite{gu2021s4, smith2022s5}:
\begin{equation}
\dot{h}(t) = A h(t) + B u(t), \qquad y(t) = C h(t),
\end{equation}
where $u(t)$ is the (post-convolution) input sequence, $h(t)$ is a hidden
state of dimension $N=16$, and $A \in \mathbb{R}^{N \times N}$ is a
learned diagonal matrix with negative entries, following the diagonal
parameterization of Mamba, which keeps the continuous-time system stable. As in Mamba \cite{gu2023mamba}, the layer makes this
formulation \emph{selective}: rather than fixing $B$, $C$, and the
discretization step size $\Delta$ for the whole sequence, each is
computed as a linear projection of the current input token $u_k$, so the
model can dynamically control what information enters and leaves the
hidden state at every position in the scan. The continuous system is
discretized via a zero-order hold parameterized by the input-dependent
$\Delta_k$,
\begin{equation}
\bar{A}_k = \exp(\Delta_k A), \qquad
\bar{B}_k = (\Delta_k A)^{-1}\big(\exp(\Delta_k A) - I\big) \Delta_k B_k,
\end{equation}
giving the discrete-time recurrence applied along the scan order,
\begin{equation}
h_k = \bar{A}_k h_{k-1} + \bar{B}_k u_k, \qquad y_k = C_k h_k .
\end{equation}
This recurrence is computed using a hardware-aware, CUDA-accelerated
parallel scan algorithm \cite{gu2023mamba}, which avoids materializing the
full sequence of hidden states $h_k$ in slow memory.
...
root, breaking ties deterministically by visiting a node's children in
...
$G$, requiring no learned parameters, in contrast to the learned Group and
...
\]
that is, Pelvis; Right Hip, Left Hip, Spine; Right Knee, Left Knee, Neck;
Right Ankle, Left Ankle, Nose, Left Shoulder, Right Shoulder; Head, Left
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  jointbox/.style={draw, rounded corners, fill=white, minimum width=0.95cm,
    minimum height=0.7cm, font=\scriptsize, align=center},
  rowlabel/.style={font=\footnotesize, align=right}
]
 \node[rowlabel] at (-1.7,0) {Naive index order:};
 \foreach \i/\lbl [count=\c from 0] in
   {0/Pel,1/RHp,2/RKn,3/RAn,4/LHp,5/LKn,6/LAn,7/Spn,8/Nck,9/Nos,10/Hd,
    11/LSh,12/LEl,13/LWr,14/RSh,15/REl,16/RWr} {
   \node[jointbox] at (\c*1.05,0) {\lbl$_{\i}$};
 }

 \node[rowlabel] at (-1.7,-1.3) {Proposed BFS order:};
 \foreach \i/\lbl [count=\c from 0] in
   {0/Pel,1/RHp,4/LHp,7/Spn,2/RKn,5/LKn,8/Nck,3/RAn,6/LAn,9/Nos,
    11/LSh,14/RSh,10/Hd,12/LEl,15/REl,13/LWr,16/RWr} {
   \node[jointbox, fill=black!8] at (\c*1.05,-1.3) {\lbl$_{\i}$};
 }
\end{tikzpicture}%
}
...
applied identically to every BiSTSSMBlock, this scan order adds no
learned parameters and no runtime cost beyond a fixed gather/scatter
operation, distinguishing it from the auxiliary topology-fusion modules
and learned scan groupings reviewed in Section~\ref{sec:lit-ssm-pose}.

\section{Loss Function}
\label{sec:method-loss}

Consistent with the architectural scope defined in
Section~\ref{sec:intro-limitations}, the network is trained with a
standard multi-term loss formulation established in prior 3D HPE work
\cite{zhang2022mixste, zhu2023motionbert, mehraban2024motionagformer}
rather than a novel loss design; the four terms below are used
without modification, so that the empirical comparison in
Chapter~\ref{ch:results} isolates the effect of the architectural
contributions of this chapter rather than the training objective. Each
term targets a different aspect of prediction quality: per-frame joint
accuracy, robustness to scale ambiguity, and temporal smoothness. Let
$\hat{Y}, Y \in \mathbb{R}^{B,T,J,3}$ denote the predicted and
ground-truth 3D pose sequences defined in
Section~\ref{sec:method-problem}.
...
the four terms:
\begin{equation}
\mathcal{L} = \lambda_{3d}\, \mathcal{L}_{3d} + \lambda_{scale}\, \mathcal{L}_{3d\text{-}scale} + \lambda_{vel}\, \mathcal{L}_{3d\text{-}velocity} + \lambda_{diff}\, \mathcal{L}_{diff},
\end{equation}
with $\lambda_{3d} = 1.0$, $\lambda_{scale} = 0.5$, $\lambda_{vel} = 20.0$,
and $\lambda_{diff} = 0.5$. The comparatively large weight on
$\mathcal{L}_{3d\text{-}velocity}$ reflects its role as the primary driver
of temporal consistency between predicted and ground-truth motion, with
$\mathcal{L}_{diff}$ providing a smaller, complementary regularization on
the smoothness of the prediction itself.

\section{Summary}
\label{sec:method-summary}

This chapter presented a dual-stack, Mamba-based architecture for
...
order otherwise used by this cross-scan with a deterministic traversal
derived directly from the skeleton's own parent-child hierarchy, closing
the research gap identified in Section~\ref{sec:lit-summary-gap} without
introducing additional learned parameters. Section~\ref{sec:method-loss}
defined the four-term loss function used to train the network end-to-end.
