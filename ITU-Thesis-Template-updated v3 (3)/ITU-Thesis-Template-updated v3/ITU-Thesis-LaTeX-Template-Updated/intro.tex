...
modeling capacity while remaining simple, deterministic, and free of
additional learned parameters. We evaluate KinecMamba on the standard
...
index or a learned grouping?} KinecMamba answers it with a deterministic
breadth-first traversal of the kinematic tree, taken in the root-to-extremity
direction of a forward kinematic pass.
...
\begin{enumerate}
  ...
  \item \textbf{To empirically evaluate KinecMamba} on the standard
    ...
    zhu2023motionbert, mehraban2024motionagformer} and state-space-based
    \cite{zhang2024posemagic, huang2025posemamba, cui2025sasmamba,
    wang2025dbmambapose} baselines.
...
\begin{itemize}
  ...
  \item \textbf{The breadth-first search kinematic-tree scan order}, a
    deterministic, parameter-free ordering of the skeleton's joints along the
    spatial scan, taken in the root-to-extremity direction of the body's
    kinematic tree \cite{loper2015smpl}. This is the thesis's answer to
    Problem 3 and its central contribution, and it differs from both the
    fixed index orders and the learned scan groupings
    \cite{nguyen2025skeletonmamba} used by prior state-space pose estimators.

  \item \textbf{A controlled scan-order ablation} that holds the architecture
    and training configuration fixed and varies only the spatial scan order,
    so that any change in accuracy can be attributed to the scan order itself
    rather than to other differences between methods.

  \item \textbf{An efficiency-tier evaluation} that places KinecMamba, at
    roughly 1.26 million parameters, alongside the smallest state-space
    baselines in the field \cite{huang2025posemamba, cui2025sasmamba} on
