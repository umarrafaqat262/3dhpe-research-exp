...
otherwise disrupt. Both works establish a principle central to this
thesis: when a state space model's strictly sequential inductive bias is
...
human motion and pose estimation directly. HumMUSS
\cite{mondal2024hummuss} was among the first to apply state space
modeling to general human motion understanding, demonstrating that an
SSM-based backbone can capture the temporal dynamics of human movement
competitively with attention-based approaches. Pose Magic
\cite{zhang2024posemagic} combined a Mamba-based sequence backbone with a
graph convolutional component in a hybrid architecture, aiming to retain
the explicit skeletal-structure bias of graph convolution
(Section~\ref{sec:lit-conv-graph}) alongside the linear-complexity temporal
modeling of Mamba.
...
provides is a scan order derived \emph{directly and deterministically}
...
\textbf{A principled, deterministic node ordering measurably improves
...
SSMs in Section~\ref{sec:lit-ssm-foundations}.
...
while preserving locality (GraphRNN), a deterministic breadth-first
...
pose estimation method, however, combines them into a deterministic,
