...
Figure~\ref{fig:results-plot} plots accuracy against model size for these
baselines.
...
\addplot[only marks, mark=*, mark size=2pt, color=blue!55] coordinates {
  ...
  (0.9,41.8) (6.7,38.1) (0.64,41.48) (4.1,39.77)
};
% KinecMamba (ours): fill in the measured MPJPE, then uncomment.
% \addplot[only marks, mark=star, mark size=4pt, color=orange!85!black]
%   coordinates { (1.26, <TODO: MPJPE>) };  % KinecMamba (ours)
\end{axis}
\end{tikzpicture}
\caption[Accuracy versus model size on Human3.6M.]{Accuracy (MPJPE, lower is
  better) against model size for the baselines of Table~\ref{tab:h36m-sota} on
  Human3.6M. KinecMamba is added at its measured MPJPE once the training run
  completes.}
