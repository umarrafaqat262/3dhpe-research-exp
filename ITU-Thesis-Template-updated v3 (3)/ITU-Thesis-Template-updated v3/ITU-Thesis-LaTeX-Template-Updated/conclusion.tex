...
parameters it reaches 41.55mm MPJPE and 34.52mm P-MPJPE. Against PoseMamba-S,
the closest baseline in both architecture and parameter budget (41.8mm /
35.0mm), these are marginally lower on both metrics at a comparable parameter
budget, and in the same range as the other compact state-space model,
SasMamba. The cross-dataset evaluation on MPI-INF-3DHP is in progress, and the
scan-order ablation was left as future work.
...
configuration. The MPI-INF-3DHP cross-dataset evaluation committed to in
Section~\ref{sec:intro-objectives} is in progress, and the scan-order
ablation designed to make that attribution directly is left as future work
...
\begin{enumerate}
  ...
  \item \textbf{Extend the ablation beyond a single model size and sequence
    ...
    that of MotionAGFormer-L, or at longer input windows,
