...
of this thesis, placing the proposed method against established baselines
on Human3.6M that span the Transformer, graph, and state-space families.
...
Table~\ref{tab:h36m-sota} places the proposed method alongside
representative baselines from Chapter~\ref{ch:literature-review}, using
each baseline's officially reported P1/P2 figures under the protocol
defined in Section~\ref{sec:results-protocol}. The accuracy of the
proposed method depends on its from-scratch training run, which has not
yet finished, so its P1 and P2 entries in the table are marked pending and
will be filled in once that run completes.
...
\caption[Comparison with the state of the art on Human3.6M.]{Comparison
  ...
  cited source, and ``TBD'' marks the proposed method's pending result.
...
\hline
MHFormer \cite{li2022mhformer}            & CVPR'22  & 351 & 30.9 & 7.0   & 43.0  & 34.4 \\
PoseFormerV2 \cite{zhao2023poseformerv2}  & CVPR'23  & 243 & 14.4 & 4.8   & 45.2  & 35.6 \\
GLA-GCN \cite{yu2023glagcn}               & ICCV'23  & 243 & 1.3  & 1.5   & 44.4  & 34.8 \\
...
Pose Magic \cite{zhang2024posemagic}      & 2024     & 243 & 14.4 & 40.6  & 37.5  & n/r \\
...
\hline
\textbf{Ours} & This work & 243 & \textbf{1.26} & n/r & \textbf{TBD} & \textbf{TBD} \\
\hline
\end{tabular}%
}
\end{table}

At approximately 1.26M parameters, the proposed method sits in the same
efficiency tier as the smallest baselines in Table~\ref{tab:h36m-sota},
alongside PoseMamba-S (0.9M), SasMamba (0.64M), and GLA-GCN (1.3M), rather
...
the proposed architecture on MPI-INF-3DHP \cite{mehta2017mpiinf3dhp} in
addition to Human3.6M, in order to assess generalization beyond a single
controlled capture setup, following the same protocol used by the
baselines in Table~\ref{tab:h36m-sota}
\cite{tang2023stcformer, zhao2023poseformerv2, huang2025posemamba}.
