...
\label{sec:intro-limitations-existing}

The problems above surface as five concrete limitations of current 2D-to-3D
lifting methods, which together motivate the design choices of this thesis:
...
\begin{enumerate}
  \item \textbf{KinecMamba}, a bidirectional spatio-temporal state space
    network for monocular 2D-to-3D pose lifting that models joint and frame
    dependencies at linear cost in the number of spatio-temporal tokens,
    answering Problems 1 and 2 of
    Section~\ref{sec:intro-problem-statement}.

  \item \textbf{The breadth-first search (BFS) kinematic-tree scan order}, an
    ordering of the skeleton's joints along the spatial scan taken in the
    root-to-extremity direction of the body's kinematic tree
    \cite{loper2015smpl}. This is the thesis's answer to Problem 3, and it
    differs from both the fixed index orders and the learned scan groupings
    \cite{nguyen2025skeletonmamba} used by prior state-space pose estimators.

  \item \textbf{An anatomy-aware bone-length loss} that supervises the lengths
    and temporal consistency of the skeleton's bones and is paired with the
    BFS scan, so that the scan order and the training objective reinforce the
    same skeletal structure.

  \item \textbf{An empirical study on Human3.6M and MPI-INF-3DHP}, including a
    controlled scan-order ablation that holds the architecture and training
    configuration fixed and varies only the spatial scan order, so that any
    change can be attributed to the scan order itself.
\end{enumerate}
