...
reaches an error marginally below its closest comparable baseline,
PoseMamba-S, at a similar parameter budget. A cross-dataset evaluation on
MPI-INF-3DHP is in progress, and a controlled scan-order ablation is
...
\begin{enumerate}
  ...
  \item \textbf{To identify the controlled scan-order ablation required to
    isolate the specific contribution of the BFS kinematic-tree scan} from
    the underlying SSM backbone, comparing it against alternative scan orders
    (e.g., fixed joint-index order, learned group/joint scan strategies), and
    to set this out as future validation.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Architectural scope.} The contributions of this thesis are
    the sequence backbone, the BFS kinematic-tree spatial scan order, and the
    anatomy-aware bone-length loss that reinforces it. Broader loss-function
    engineering, data augmentation strategies, temporal
    post-processing/smoothing, and multi-hypothesis modeling
    \cite{li2022mhformer} are outside the scope of this work, except where a
    standard, established choice is adopted without modification to support
