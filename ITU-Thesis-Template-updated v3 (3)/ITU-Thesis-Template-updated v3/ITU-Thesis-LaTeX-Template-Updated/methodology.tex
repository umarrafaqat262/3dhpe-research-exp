...
\begin{figure}[t]
\centering
\IfFileExists{figures/kinematic_tree.pdf}{%
  \includegraphics[width=0.85\textwidth]{figures/kinematic_tree.pdf}%
}{%
  \fbox{\parbox[c][5cm][c]{0.85\textwidth}{\centering\textit{[Insert
    \texttt{figures/kinematic\_tree.pdf}: full-body skeleton kinematic tree, 17 joints,
    pelvis as root. See \texttt{figure\_prompts.md} (Prompt 2).]}}}%
}
...
\IfFileExists{figures/architecture.pdf}{%
  ...
  \fbox{\parbox[c][5cm][c]{0.9\textwidth}{\centering\textit{[Insert
    \texttt{figures/architecture.pdf}: end-to-end pipeline, 2D pose input to 3D pose
    output. See \texttt{figure\_prompts.md} (Prompt 1).]}}}%
}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/bistssm_layer.pdf}{%
  \includegraphics[width=0.6\textwidth]{figures/bistssm_layer.pdf}%
}{%
  \fbox{\parbox[c][5cm][c]{0.6\textwidth}{\centering\textit{[Insert
    \texttt{figures/bistssm\_layer.pdf}: BiSTSSM layer, cross-scan to $\times$4 selective
    SSM to cross-merge. See \texttt{figure\_prompts.md} (Prompt 3).]}}}%
}
...
\begin{figure}[t]
\centering
\IfFileExists{figures/scan_order.pdf}{%
  \includegraphics[width=\textwidth]{figures/scan_order.pdf}%
}{%
  \fbox{\parbox[c][3cm][c]{0.95\textwidth}{\centering\textit{[Insert
    \texttt{figures/scan\_order.pdf}: two strips, naive index order vs BFS kinematic order.
    See \texttt{figure\_prompts.md} (Prompt 4).]}}}%
}
