...
\begin{figure}[t]
\centering
\includegraphics[height=0.42\textheight]{figures/kinematic_tree.png}
...
\begin{figure}[t]
\centering
\includegraphics[height=0.5\textheight]{figures/bistssm_layer.png}
...
\begin{figure}[t]
\centering
% Drawn in TikZ (exact joint indices). The AI-generated draft of this figure had
% label errors (missing joint 13, wrong subscripts), so the vector version is used.
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
...
accuracy and computational efficiency on Human3.6M against the baselines
identified in Chapter~\ref{ch:literature-review}, with a cross-dataset
evaluation on MPI-INF-3DHP in progress and the scan-order ablation that would
isolate the specific contribution of the BFS kinematic-tree scan order set out
as future work.
